 ...
 *
 * This is local version of genulens v1.2. (use NR ran1 and C)
 *
 ...
 *   Add PtEthEfile option to conduct Bayesian analysis for FFP events in NK+
 *
 * */
#include <math.h> 
#include <stdio.h> 
...
static char *inputdir; // store input_files 
...
int main(int argc,char **argv)
{
  ...
  char curdir[200];
  getcwd(curdir, 200);
  inputdir = (strstr(curdir, "Users")) ? "/Users/koshimoto/work/Likelihood/input_files"
           : "/moao29_4/koshimoa/binfit/work/Likelihood/input_files";

  //--- read parameters ---
  int CheckD   = getOptiond(argc,argv,"CheckD", 1, 0);
  // long seed    = getOptioni(argc,argv,"seed", 1, 12304357); // seed of random number
  seed    = getOptioni(argc,argv,"seed", 1, -2); // seed of random number
  long seed0 = seed;
  idum = &seed;
  // gsl_rng_env_setup();
  // T = gsl_rng_default;
  ...
  //nunote: 初期質量がMiniWDmaxより大きければ乱数次第でBHがNSになる。質量はある程度ランダムに決まる。
  ...
  char file_MLemp[200];
  sprintf(file_MLemp, "%s/MLemp.dat", inputdir);
  ...
  char fileVc[200];
  sprintf(fileVc, "%s/Rotcurve_BG16.dat", inputdir);
  ...
  if (ND == 3){ // More Sormani+21-like NSD, Use input_files/NSD_moments.dat 
    ...
    char fileND[200];
    sprintf(fileND, "%s/NSD_moments.dat", inputdir);
  ...
  /***************************************************************/
  // Read file for probability distribution of (tE, thetaE) (usually from MCMC) 
  char *PtEthEfile;
  PtEthEfile    = getOptions(argc,argv, "PtEthEfile", 1, "");
  double dlogtE_P   = getOptiond(argc,argv, "dlogtE_P", 1, 0.025);
  double dlogthE_P  = getOptiond(argc,argv, "dlogthE_P", 1, 0.025);

  // from get_chi2_IMF_tEthEMOA_both.c
  double logtEst_P = -99, logtEen_P, *logthEst_P, *logthEen_P, **LMCMC_P;
  char line2[8000];
  char *words2[1000];
  FILE *fp2;
  if ((fp2=fopen(PtEthEfile,"r"))!=NULL){
    int nlogthE_Pmax = 0;
    double logtE, logtE0;
    double logthEmin_P = 99, logthEmax_P = -99;
    while (fgets(line2,1000,fp2) !=NULL){
      split(" ", line2, words2);
      if (*words2[0] == '#') continue;
      // logtEst_P と logtEen_Pを保存
      if (logtEst_P == -99){
        logtEst_P = atof(words2[0]);
        logtE = logtE0 = logtEst_P;
      }
      logtEen_P = atof(words2[0]); // overwritten by last line2
      // LMCMC_Pのmalloc割り当て範囲取得のため、nlogthE_Pmaxを調査
      logtE = atof(words2[0]);
      if (logtE != logtE0){
        double tmp = (logthEmax_P - logthEmin_P)/dlogthE_P + 1.5;
        if (tmp > nlogthE_Pmax)
          nlogthE_Pmax = tmp;
        logthEmin_P =  99;
        logthEmax_P = -99;
      }
      if (atof(words2[1]) < logthEmin_P) logthEmin_P = atof(words2[1]);
      if (atof(words2[1]) > logthEmax_P) logthEmax_P = atof(words2[1]);
      logtE0 = logtE;
    }
    // printf("%s : nlogthEmax= %d\n",words[1],nlogthE_Pmax);
    // logthEst_P, logthEen_P, LMCMC_P を定義。
    int nlogtE_P = (logtEen_P - logtEst_P)/dlogtE_P + 1.5;
    logthEst_P = calloc(nlogtE_P, sizeof(double *));
    logthEen_P = calloc(nlogtE_P, sizeof(double *));
    LMCMC_P    = malloc(sizeof(double *) * nlogtE_P);
    for (int j = 0; j < nlogtE_P; j++){
      LMCMC_P[j] = calloc(nlogthE_Pmax, sizeof(double *));
    }
    fclose(fp2);
    if ((fp2=fopen(PtEthEfile,"r"))==NULL){
      printf("can't open %s\n",PtEthEfile);
    ...
    while (fgets(line2,1000,fp2) !=NULL){
      ...
      if (*words2[0] == '#') continue;
      logtE = atof(words2[0]);
      double logthE = atof(words2[1]);
      int ilogtE  = (logtE  - logtEst_P)/dlogtE_P + 0.5;
      if (logthEst_P[ilogtE] == 0) logthEst_P[ilogtE] = logthE;
      logthEen_P[ilogtE] = logthE;
      int ilogthE = (logthE - logthEst_P[ilogtE])/dlogthE_P + 0.5;
      LMCMC_P[ilogtE][ilogthE] = atof(words2[2]);
    }
    fclose(fp2);
    // 読み込めてるのかのテスト。読み込めてそう。出力結果を
    // plotxy.py -file tmp3.dat -xcol 0 -ycol 1 -zcol 2 -cmap jet -LOGZ -xmin -1.60 -xmax 0.20 -ymin -3.50 -ymax -0.60 -SIZE 0.1
    // とかでプロットすると結構楽しい。
    double tEst_P = pow(10.0,logtEst_P), tEen_P = pow(10.0,logtEen_P+dlogtE_P);
    // printf("#logtE logthE      L_MCMC\n");
    // for (int i=0; i<nlogtE_P; i++){
    //   double logtE = logtEst_P + i*dlogtE_P;
    //   printf("# logtE= %f : logthEst= %f logthEen= %f\n",logtE,logthEst_P[i],logthEen_P[i]);
    //   int nlogthE_P = (logthEen_P[i] - logthEst_P[i])/dlogthE_P + 1.5;
    //   for (int j=0; j<nlogthE_P; j++){
    //     double logthE = logthEst_P[i] + j*dlogthE_P;
    //     printf("%6.3f %6.3f %.5e\n",logtE,logthE,LMCMC_P[i][j]);
    //   }
    // }
  }
  ...
  if (CheckD == 1){
    ...
    for (int i=0; i<5000000; i++){
      ...
      double vxyz_L[3] = {};
      double tau_l = (j_L == 9) ? mageND + sageND*gasdev(idum)
                   : (j_L == 8) ? mageB + sageB*gasdev(idum) 
                   : (j_L ==10) ? 14 
                   : medtauds[j_L];
...
//-------------nunota define histgram--------------//

double MIN_MU = 0.0;
double MAX_MU = 20.0;
double MU_BIN_WIDTH = 0.5;

double PHI_MIN = -PI;
double PHI_MAX = PI;
double PHI_BIN_WIDTH = 5 * PI / 180;

int MU_BIN_NUM = (int)((MAX_MU - MIN_MU) / MU_BIN_WIDTH);
int PHI_BIN_NUM = (int)((PHI_MAX - PHI_MIN) / PHI_BIN_WIDTH);


double DL_MIN = 0.0;
double DL_MAX = 12000.0;
double DL_BIN_WIDTH = 500.0;

double DS_MIN = 0.0;
double DS_MAX = 16000.0;
double DS_BIN_WIDTH = 500.0;

int DL_BIN_NUM = (int)((DL_MAX - DL_MIN) / DL_BIN_WIDTH);
int DS_BIN_NUM = (int)((DS_MAX - DS_MIN) / DS_BIN_WIDTH);

printf("# BIN_INFO: MU_MIN= %.2f MU_MAX= %.2f MU_WIDTH= %.2f MU_BIN_NUM= %d ", MIN_MU, MAX_MU, MU_BIN_WIDTH, MU_BIN_NUM);
printf("PHI_MIN= %.2f PHI_MAX= %.2f PHI_WIDTH= %.4f PHI_BIN_NUM= %d ", PHI_MIN, PHI_MAX, PHI_BIN_WIDTH, PHI_BIN_NUM);
printf("DL_MIN= %.1f DL_MAX= %.1f DL_WIDTH= %.1f DL_BIN_NUM= %d ", DL_MIN, DL_MAX, DL_BIN_WIDTH, DL_BIN_NUM);
printf("DS_MIN= %.1f DS_MAX= %.1f DS_WIDTH= %.1f DS_BIN_NUM= %d\n", DS_MIN, DS_MAX, DS_BIN_WIDTH, DS_BIN_NUM);
printf("# DS DL mu mu_hist phi phi_hist\n");
//-------------------------------------------------//
  ...
  for (int i_DL = 0; i_DL < DL_BIN_NUM; i_DL++) {
    double DL_bin_center = DL_MIN + (i_DL + 0.5) * DL_BIN_WIDTH;
  for (int j_DS = 0; j_DS < DS_BIN_NUM; j_DS++) {
    double DS_bin_center = DS_MIN + (j_DS + 0.5) * DS_BIN_WIDTH;
    if (DS_bin_center <= DL_bin_center) continue;
  int *mu_hist = (int *)calloc(MU_BIN_NUM, sizeof(int));   
  int *phi_hist = (int *)calloc(PHI_BIN_NUM, sizeof(int));
  int total_count = 0;
  ...
  for (long j=0; j< NSIMU; j++){
     ...
     double tau_s = (i_s == 9) ? mageND + sageND*gasdev(idum)
                  : (i_s == 8) ? mageB + sageB*gasdev(idum) 
                  : (i_s ==10) ? 14 
                  : medtauds[i_s];
     ran = ran1(idum);
     inttmp = ran*20;
     ...
     double tau_l = (i_l == 9) ? mageND + sageND*gasdev(idum)
                  : (i_l == 8) ? mageB + sageB*gasdev(idum)
                  : (i_l == 10) ? 14
                  : medtauds[i_l];
     ran = ran1(idum) * (cumu_rho_L[i_l][nbinDlmax] - cumu_rho_L[i_l][nbinDlmin]) + cumu_rho_L[i_l][nbinDlmin];
     ...
     // ==== nunota D_S をビンの範囲でランダムに振る ====
     double DS_min_bin = DS_MIN + j_DS * DS_BIN_WIDTH;
     double DS_max_bin = DS_min_bin + DS_BIN_WIDTH;
     D_s = DS_min_bin + ran1(idum) * (DS_max_bin - DS_min_bin);
     // ==== nunota D_L をビンの範囲でランダムに振る ====
     double DL_min_bin = DL_MIN + i_DL * DL_BIN_WIDTH;
     double DL_max_bin = DL_min_bin + DL_BIN_WIDTH;
     D_l = DL_min_bin + ran1(idum) * (DL_max_bin - DL_min_bin);
     ...
     // double phikick   = (REMNANT == 1 && onlyWD == 0) ? ran1(idum)*2*PI : 0;  // not correct
     // double thetakick = (REMNANT == 1 && onlyWD == 0) ? ran1(idum)*PI : 0; // not correct
     double thetakick = (REMNANT == 1 && onlyWD == 0) ? asin(1 - 2*ran1(idum)) : 0;
     double phikick   = (REMNANT == 1 && onlyWD == 0) ? ran1(idum)*2*PI - PI : 0;
     ...
     for (int iREM = 0; iREM < nREM; iREM++){
       ...
       murells[iREM] = murellhel; // hel -> geo, iru
       murelbs[iREM] = murelbhel; // hel -> geo, iru
     ...
     if (nbinMmax - nbinMmin > 0){  // importance sampling
       ran = ran1(idum) * (PlogM_cum_norm_B[nbinMmax] - PlogM_cum_norm_B[nbinMmin]) + PlogM_cum_norm_B[nbinMmin]; // limit Mmin - Mmax
     ...
     if (M_l > Minidie){ //nunote: ここで死んだかどうかを判定。死んだ星を入れないオプションだとそのままreject. 主系列星のMFはIMFから死んだやつを除いてできる
     ...
     if (BINARY && fREM == 0){  // Remnant in a binary should be ideally considered, but currently not yet
       ...
       if (ran < 0.5 * mult){ // close binary
         ...
         q2 = pow( (1-tmp)*ran1(idum) + tmp, 1/(gamma+1) ); // inverse transform sampling
       }else if(ran < mult){
       ...
       if (swl > 0){
         ...
         u0S = (u0obs > 0) ? u0obs : ran1(idum);  // u0 following uniform distribution if not given
         double tmp = qtmp/u0S; 
...
//nunota count------------
     double phi = atan2(piEE, piEN);  

     if (murel >= MIN_MU && murel < MAX_MU && phi >= PHI_MIN && phi < PHI_MAX) {
         int mu_bin = (int)((murel - MIN_MU) / MU_BIN_WIDTH);
         if (mu_bin >= 0 && mu_bin < MU_BIN_NUM) {
             mu_hist[mu_bin]++;
         }
         else{
             continue;
         }
         int phi_bin = (int)((phi - PHI_MIN) / PHI_BIN_WIDTH);
         if (phi_bin >= 0 && phi_bin < PHI_BIN_NUM) {
             phi_hist[phi_bin]++;
         }
         else{
             continue;
...
//---------------------------
     ...
     // }
     if (Gamma < ran1(idum) && SMALLGAMMA == 0){
       j--;
       continue;
     }
     // if (i_l == 10)
     ...
     // Multiply input P(tE, thE) if exist
     if (logtEst_P != -99){
       double logtE  = log10(tE);
       double logthE = log10(thetaE);
       int ilogtE_P  = (logtE  - logtEst_P)/dlogtE_P;
       int ilogthE_P = (logthE - logthEst_P[ilogtE_P])/dlogthE_P;
       double P_MCMC = (logtE  > logtEst_P  && logtE < logtEen_P + dlogtE_P 
                     && logthE > logthEst_P[ilogtE_P] && logthE < logthEen_P[ilogtE_P] + dlogthE_P) ? LMCMC_P[ilogtE_P][ilogthE_P] : 0;
       if (P_MCMC == 0){
         like_tE = 0;
         like_thetaE = 0;
       }
...
//------nunota output
  int max_bin_num = (MU_BIN_NUM > PHI_BIN_NUM) ? MU_BIN_NUM : PHI_BIN_NUM;
  
  for (int k = 0; k < ((MU_BIN_NUM > PHI_BIN_NUM) ? MU_BIN_NUM : PHI_BIN_NUM); k++) {
      double mu_bin_center = (k < MU_BIN_NUM) ? (MIN_MU + (k + 0.5) * MU_BIN_WIDTH) : 0.0;
      double mu_norm = (k < MU_BIN_NUM && total_count > 0) ? ((double)mu_hist[k] / total_count) : 0.0;
  
      double phi_bin_center = (k < PHI_BIN_NUM) ? (PHI_MIN + (k + 0.5) * PHI_BIN_WIDTH) : 0.0;
      double phi_norm = (k < PHI_BIN_NUM && total_count > 0) ? ((double)phi_hist[k] / total_count) : 0.0;
  
      printf("%.3f %.2f %.3f %.6f %.3f %.6f\n",DS_bin_center, DL_bin_center, mu_bin_center, mu_norm, phi_bin_center, phi_norm);
  }
  }
  }
//-----------------
...
void store_IMF_nBs(int B, double *logMass, double *PlogM, double *PlogM_cum_norm, int *imptiles, double M0, double M1, double M2, double M3, double Ml, double Mu, double alpha1, double alpha2, double alpha3, double alpha4, double alpha0){
  ...
  FILE *fp;
  char file1[200];
  sprintf(file1, "%s/Minidie.dat", inputdir);
...
void Mini2Mrem (double *pout, double Mini, int mean) {  // mean = 1: give mean, 0: give random
  ...
  if (Mini < MiniWDmax){ //nunote: 初期質量がMiniWDmaxより小さければWDになる。その際の質量は決定論的に決まる
     Mrem = 0.109 * Mini + 0.394; // IFMR from Kalirai+08
     fREM = 1; // WD
  }else{ //nunote: 初期質量がMiniWDmaxより大きければ乱数次第でBHがNSになる。質量はある程度ランダムに決まる。
     ...
     do {
       MNS = (Mini < 13.0) ? 2.24 + 0.508 *(Mini - 14.75) 
           ...
           : (Mini < 21.7 && mean == 0) ? 1.60 + 0.158*gasdev(idum)
  ...
  while(1){
    ...
    for (int j=0;j<3;j++){ // Find Pmax by Newton's method (search dP/dfg = 0)
      ...
      if (j==2 && fabs(dPdfgc/Pmax) > 0.1){
        nj++;
        fg1 = (dPdfgc > 0) ? fgc + 0.05/nj*ran1(idum) : fgc - 0.05/nj*ran1(idum);
        j = -1;
        continue;
      }
      if (dPdfg1 == 0){ // too left or too right
        jj = (dPdfgc == 0) ? 0.5 : 0.2*ran1(idum);
...
//----------------
void get_vxyz_ran(double *vxyz, int i, double tau, double D, double lD, double bD) //nunota 目印
{
  void Dlb2xyz(double D, double lD, double bD, double Rsun, double *xyz);
  double getcumu2xist (int n, double *x, double *F, double *f, double Freq, int ist, int inv);
  double getx2y(int n, double *x, double *y, double xin);
  ...
  if (i < 8 && B14disk == 1){
    ...
    do{
      double vR   =    0 + gasdev(idum)*sigU; // radial velocity
      double vphi = aveV + gasdev(idum)*sigV;
      vx = -vphi * y/R + vR * x/R; // x/R = cosphi, y/R = sinphi
      vy =  vphi * x/R + vR * y/R; // x/R = cosphi, y/R = sinphi
      vz =    0 + gasdev(idum)*sigW; // vertical velocity
    ...
    do{
      double vphi = m_vphi + gasdev(idum)*sigphi; // Assume vphi distribution is symmetrical (which is not true)
      double vR = gasdev(idum)*sigR;
      vx = -vphi * y/R + vR * x/R; // x/R = cosphi, y/R = sinphi
      vy =  vphi * x/R + vR * y/R; // x/R = cosphi, y/R = sinphi
      vz =  facR * vR  + gasdev(idum)*sigz_R; // random w/ correlation coeff
      // printf("%f %f %f %f %f %f %f %f %f %f\n",R,z,vphi,vR,vz,corRz,m_vphi,sigphi,sigR,sigz);
    }while (vx*vx + vy*vy + vz*vz > vescb*vescb);
  }else if (i == 10){ // Stellar halo
    double vR   = sigU_SH * gasdev(idum);
    double vphi = sigV_SH * gasdev(idum);
    vx = -vphi * y/R + vR * x/R; // x/R = cosphi, y/R = sinphi
    vy =  vphi * x/R + vR * y/R; // x/R = cosphi, y/R = sinphi
    vz =    0 + gasdev(idum)*sigW_SH; // vertical velocity
    ...
    do{
      vx = - vrot * y/R + avevxb * costheta + sigx * gasdev(idum);
      vy =   vrot * x/R + avevxb * sintheta + sigy * gasdev(idum);
      vz =                                    sigz * gasdev(idum);
    }while (vx*vx + vy*vy + vz*vz > vescb*vescb);
  }
  vxyz[0] = vx;
...
void getaproj(double *pout, double M1, double M2, int coeff)  { // pick up aproj  
   ...
   double ran = coeff * fabs(gasdev(idum));
...
int make_LFs(double *MIs, double **CumuN_MIs, double *logMass, double *PlogM_cum_norm) 
{
   ...
   int i=0;
   char infile[200];
   sprintf(infile, "%s/NbleNall_bin.dat", inputdir);
   if((fp=fopen(infile,"r"))==NULL){
      printf("can't open %s\n",infile);
      exit(1);
   }
   ...
   for (int icomp=0; icomp<ncomp; icomp++){
     (icomp == 0) ? sprintf(file1, "%s/iso-thin1.dat", inputdir) :
     (icomp == 1) ? sprintf(file1, "%s/iso-thin2.dat", inputdir) :
     (icomp == 2) ? sprintf(file1, "%s/iso-thin3.dat", inputdir) :
     (icomp == 3) ? sprintf(file1, "%s/iso-thin4.dat", inputdir) :
     (icomp == 4) ? sprintf(file1, "%s/iso-thin5.dat", inputdir) :
     (icomp == 5) ? sprintf(file1, "%s/iso-thin6.dat", inputdir) :
     (icomp == 6) ? sprintf(file1, "%s/iso-thin7.dat", inputdir) :
     (icomp == 7) ? sprintf(file1, "%s/iso-thick2.dat", inputdir)  :
     (icomp == 8) ? sprintf(file1, "%s/iso-bar_age.dat", inputdir) :
     (icomp == 9) ? sprintf(file1, "%s/iso-NSD.dat", inputdir) :
                    sprintf(file1, "%s/iso-halo.dat", inputdir);
     if((fp=fopen(file1,"r"))==NULL){
        printf("can't open %s\n",file1);
        exit(1);
     }
...
//---------------nunota  これが大事！！calc_rho_eachで星密度計算している。Dlb2xyzとかも使えそう---
void calc_rho_each(double D, int idata, double *rhos, double *xyz, double *xyb){  // return rho for each component 
  void Dlb2xyz(double D, double lD, double bD, double Rsun, double *xyz);
  double calc_rhoB(double xb, double yb, double zb);
