...
int main(int argc,char **argv)
{
  ...
  for (int ibin=0; ibin<=nbin; ibin++){
    ...
    for (int i=0;i<ncomp;i++){
      ...
      rhoD_L[i][ibin] =  rho;
      //rhoD_L[i][ibin] = rho * 1e-06 * D[ibin] * D[ibin];
  ...
  int test_rho_L = 0;//test by nunota
  if (test_rho_L == 1){
    for (int ibin=0; ibin<=nbin; ibin++){
    double sum_n_all = 0;
    double sum_rhoD_S = 0;
    ...
    printf("%.2f ",D[ibin]);
      for (int i=0;i<ncomp;i++){
        printf("%.3e ",rhoD_L[i][ibin]);
        sum_n_all += rhoD_L[i][ibin];
      }
      printf("%.3e ",sum_n_all);
      for (int i=0;i<ncomp;i++){
        printf("%.3e ",rhoD_S[i][ibin]);
        sum_rhoD_S += rhoD_S[i][ibin];
      }
      printf("%.3e ",sum_rhoD_S);
    printf("\n");
    }
  exit(1);
  }
  ...
  //-------------test nunota----------//
double n_all_each[ncomp];
double rhoD_S_each[ncomp];
void get_number_density_nMS_at_Dlb(double D_pc, double l_deg, double b_deg, double *nMS_out);
double get_rhoD_S_at_Dlb(double D_pc, double l_deg, double b_deg, double *rhoD_S_out,
                            double AI0, double Isen, double Isst,
                            double EVI0, double VIsen, double VIsst,
                            double gammaDs, double hscale);

double D_tmp = 0.0;
for (int j = 0; D_tmp <= 16000; j++) {
    double sum_n_all = 0;
    double sum_rhoD_S = 0;

    // 全星数密度（主系列星数）
    get_number_density_nMS_at_Dlb(D_tmp, lSIMU, bSIMU, n_all_each);

    // ソース星 rhoD_S を一括計算
    get_rhoD_S_at_Dlb(D_tmp, lSIMU, bSIMU, rhoD_S_each,
                           AI0, Isen, Isst, EVI0, VIsen, VIsst, gammaDs, hscale);

    printf("%.2f ", D_tmp);

    // 全星数密度出力
    for (int i = 0; i < ncomp; i++) {
        sum_n_all += n_all_each[i];
        printf("%.3e ", n_all_each[i]);
    }
    printf("%.3e ", sum_n_all);  // 全星合計

    // rhoD_S 出力
    for (int i = 0; i < ncomp; i++) {
        sum_rhoD_S += rhoD_S_each[i];
        printf("%.3e ", rhoD_S_each[i]);
    }
    printf("%.3e\n", sum_rhoD_S);  // rhoD_S 合計
...
// コア関数：各成分の質量密度を取得
void get_mass_density_nu_at_Dlb(double D_pc, double l_deg, double b_deg, double *nu_out) {
    double xyz[3], xyb[2];
    double x, y, z, R, xb, yb, zb;
    double xn, yn, zn, rs, rhotmp, zdtmp;
    void Dlb2xyz(double D, double l_deg, double b_deg, double Rsun, double *xyz);
    double calc_rhoB(double xb, double yb, double zb);

    // 初期化
    for (int i = 0; i < ncomp; i++) nu_out[i] = 0.0;

    // 座標変換
    Dlb2xyz(D_pc, l_deg, b_deg, R0, xyz);
    x = xyz[0]; y = xyz[1]; z = xyz[2];
    R = sqrt(x * x + y * y);
    ...
    if (DISK > 0) {
        for (int idisk = 0; idisk < 8; idisk++) {
            ...
            zdtmp = (R > 4500)
                ? zd[idisk] + (R - R0) * (zd[idisk] - zd45[idisk]) / (R0 - 4500)
                : zd45[idisk];

            rhotmp = (idisk < 7)
                ? 4.0 / (exp(2 * z / zdtmp) + exp(-2 * z / zdtmp) + 2)
                : exp(-fabs(z) / zd[idisk]);

            rhotmp *= zd[idisk] / zdtmp;

            if (DISK == 1)
                rhotmp *= exp(-R / Rd[itmp] - pow(Rh / R, nh));
            else if (DISK == 2)
                rhotmp *= (R > Rdbreak) ? exp(-R / Rd[itmp]) : exp(-Rdbreak / Rd[itmp]);
            else if (DISK == 3)
                rhotmp *= exp(-R / Rd[itmp]);

            nu_out[idisk] = rhotmp*rho0d[idisk];
        }
    }
    ...
    yb = -x * sintheta + y * costheta;
    zb =  z;
    nu_out[8] = calc_rhoB(xb, yb, zb)* rho0b;  // Already returns mass density

    // ======== NSD (i = 9) ==========
    if (ND > 0) {
        if (ND == 3) {
            if (R <= RenND - 30 && fabs(z) <= zenND - 20) {
                nu_out[9] = pow(10.0, interp_xy(nzND, nRND, logrhoNDs, zstND, RstND, dzND, dRND, fabs(z), R));
            } else {
                nu_out[9] = 0;
            }
        } else {
            ...
            yn = fabs(yb / y0ND);
            zn = fabs(zb / z0ND);
            rs = pow(pow(xn, C1ND) + pow(yn, C1ND), 1.0 / C1ND) + zn;
            nu_out[9] = exp(-rs)* rho0ND;
        }
    }
    ...
    double aSHsun_inv = R0 / sqrt(aSH2);
    nu_out[10] = pow(aSHsun_inv, alphaSH)* rho0SH;
}

//主系列星の数密度を返す
void get_number_density_nMS_at_Dlb(double D_pc, double l_deg, double b_deg, double *nMS_out) {
    double xyz[3], xyb[2];
    double rhos[ncomp];  // normalized spatial profiles
    void calc_rho_each(double D, int idata, double *rhos, double *xyz, double *xyb);

    // 呼び出し元が使う地点情報（銀経・銀緯）は idata に格納されている前提
    int idata = 0;
    calc_rho_each(D_pc, idata, rhos, xyz, xyb);

    // 各成分の主系列星数密度を計算（stars / pc^3）
    for (int i = 0; i < ncomp; i++) nMS_out[i] = 0.0;

    for (int i = 0; i <= 7; i++) {
        nMS_out[i] = n0MSd[i] * rhos[i];  // Thin & Thick Disk
    }
    nMS_out[8]  = n0MSb  * rhos[8];  // Bulge
    nMS_out[9]  = n0MSND * rhos[9];  // Nuclear Disk
    nMS_out[10] = n0MSSH * rhos[10]; // Stellar Halo
}
...
double get_rhoD_S_at_Dlb(double D_pc, double l_deg, double b_deg, double *rhoD_S_out,
    ...
    // 減光計算
    double extI  = AI0 * (1.0 - exp(-D_pc / hscale)) + 5.0 * log10(0.1 * (D_pc + 0.1));
    double extVI = EVI0 * (1.0 - exp(-D_pc / hscale));
    ...
    for (int i = 0; i < ncomp; i++) {
        ...
        if (AI0 > 0 && Isen - Isst > 0 && EVI0 > 0 && VIsen - VIsst > 0) {
            double fIVIs = fIVI_detect(extI, Isst, Isen, extVI, VIsst, VIsen, i);
            rhoD_S = nMS * fIVIs * 1e-06 * D_pc * D_pc;
        } else if (AI0 > 0 && Isen - Isst > 0) {
            double fIs = fLF_detect(extI, Isst, Isen, i);
            rhoD_S = nMS * fIs * 1e-06 * D_pc * D_pc;
        } else {
            double tmpDswt = (gammaDs == 0.5) ? sqrt(D_pc / 8000.0)
                               : pow(((D_pc + 10.0) / 8000.0), fabs(gammaDs));
            if (gammaDs < 0) tmpDswt = 1.0 / tmpDswt;
            rhoD_S = nMS * tmpDswt * 1e-03;
        }
