...
int main(int argc,char **argv)
{
  ...
  int REMNANT     = getOptiond(argc,argv,"REMNANT",  1,  1);
  ...
  //---nunota define mass hist---//
  double M_MIN = 0.0;
  double M_MAX = 15;
  double M_BIN_WIDTH = 0.01;
  printf("# M_MIN= %f M_MAX= %f M_BIN_WIDTH= %f \n",M_MIN,M_MAX,M_BIN_WIDTH);
  int MBIN_NUM = (int)((M_MAX - M_MIN) / M_BIN_WIDTH);
  int FREM_NUM = 4;
  
  int ***M_hist;
  int Ntotal = 0;
  
  M_hist = (int ***)malloc(sizeof(int **) * ncomp);
  
  for (int i = 0; i < ncomp; i++) {
      M_hist[i] = (int **)malloc(sizeof(int *) * FREM_NUM);
      
      for (int f = 0; f < FREM_NUM; f++) {
          M_hist[i][f] = (int *)calloc(MBIN_NUM, sizeof(int));
      }
  }
  ...
  for (long j=0; j< NSIMU; j++){
     ...
     int test_Morg = 0;//test by nunota
     if (test_Morg){
     printf("%.3e %.3e %d\n",Morg,M_l,i_l);
     }
    
    int Mbin = (int)((M_l - M_MIN) / M_BIN_WIDTH);
    
    if (Mbin >= 0 && Mbin < MBIN_NUM && i_l >= 0 && i_l < ncomp && fREM >= 0 && fREM < FREM_NUM) {
        M_hist[i_l][fREM][Mbin]++;
        Ntotal++;
    }
     continue;
...
//---nunota print result----//
for (int f = 0; f < FREM_NUM; f++) {
    for (int b = 0; b < MBIN_NUM; b++) {
        double Mcenter = M_MIN + (b + 0.5) * M_BIN_WIDTH;  // ビンの中心値
        printf("%.3f ", Mcenter);  // 質量ビン中心

        // 各 i_l の hist 出力
        for (int i = 0; i < ncomp; i++) {
            double M_dens = (double) M_hist[i][f][b]/Ntotal;
            printf("%.3e ", M_dens);
        }
