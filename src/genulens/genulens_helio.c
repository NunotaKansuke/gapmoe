...
int main(int argc,char **argv)
{
  ...
  int HistDs   = getOptiond(argc,argv,"HistDs", 1, 0);
  ...
  if ((fp2=fopen(PtEthEfile,"r"))!=NULL){
    ...
    // とかでプロットすると結構楽しい。
    printf("#-------------- Input P(logtE, logthE) file ---------------\n");
    printf("#   Read %s\n",PtEthEfile);
    printf("#     (logtEst, logtEen, dlogtE)_P= (%.3f %.3f %.3f)\n", logtEst_P, logtEen_P, dlogtE_P);
    double tEst_P = pow(10.0,logtEst_P), tEen_P = pow(10.0,logtEen_P+dlogtE_P);
    printf("#     (tEmid, tEe)= (%.3f %.3f)\n", 0.5*(tEst_P+tEen_P), 0.5*(-tEst_P+tEen_P));
  ...
  if (HistDs == 1){
    #define DS_MIN 0
    #define DS_MAX 15000
    #define DS_BIN_WIDTH 10
    #define N_DS_BIN ((DS_MAX - DS_MIN) / DS_BIN_WIDTH)
    
    int hist_ds[N_DS_BIN] = {0};  
    double getcumu2xist (int n, double *x, double *F, double *f, double Freq, int ist, int inv);
    for (int i=0; i<100000000; i++){
      ...
      double d_S = getcumu2xist(nbin+1, D, cumu_rho_S[j_S],rhoD_S[j_S],ran*cumu_rho_S[j_S][nbin],kst,0);
      int bin_ds = (int)((d_S - DS_MIN) / DS_BIN_WIDTH);
      if (bin_ds >= 0 && bin_ds < N_DS_BIN) {
          hist_ds[bin_ds]++;
      }
    }

    for (int i = 0; i < N_DS_BIN; i++) {
        double d_low = DS_MIN + i * DS_BIN_WIDTH;
       double d_high = d_low + DS_BIN_WIDTH;
        printf("%.1f %.1f %d\n", d_low, d_high, hist_ds[i]);
    }
    exit(1);
  }
