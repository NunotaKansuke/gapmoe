...
class DistanceDensityTable:
    ...
    def lens_pdf_given_source(self, dl_pc: float, ds_pc: float) -> float:
        ...
        if norm <= 0.0:
            return 0.0
        val = np.interp(dl_pc, self.distance_pc, self.lens_density_total, left=0.0, right=0.0)
        return float(val / norm)

    def _lens_integral_until(self, ds_pc: float) -> float:
...
class HistogramDensity(DensityModel):
    ...
    def density(self, ML: float, DL: float, DS: float, mu_N: float, mu_E: float) -> float:
        """Return density with respect to dML dDL dDS dmu_N dmu_E."""
        mu = hypot(mu_N, mu_E)
        if mu <= 0.0:
            return 0.0
        phi = atan2(mu_E, mu_N)
    ...
    def density_mu_phi(self, mass: float, dl_pc: float, ds_pc: float, mu: float, phi: float) -> float:
        """Return density with respect to dML dDL dDS dmu dphi."""
    ...
    def log_density_mu_phi(self, mass: float, dl_pc: float, ds_pc: float, mu: float, phi: float) -> float:
