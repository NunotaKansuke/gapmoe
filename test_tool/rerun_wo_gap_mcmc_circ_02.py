...
init_params = np.array([t0_true,tE_true,u0_true,rho_true,q_true,s_true,alpha_true,piEN_true,piEE_true,gamma1_true,gamma2_true,gamma3_true])
...
pre_path = "/moao38_7/nunota/gapmoe/test_result/copy/wo_gap_simu_02_circular_chain_01.h5"
pre_sampler = emcee.backends.HDFBackend(pre_path)
flat_chain = pre_sampler.get_chain(flat=True)
flat_prob = pre_sampler.get_log_prob(flat=True)
data = np.hstack([flat_chain, flat_prob[:, None]])  # (N, ndim+1)
unique_data = np.unique(data, axis=0)
unique_chain = unique_data[:, :-1]
unique_prob = unique_data[:, -1]
N_select = 24
top_indices = np.argsort(unique_prob)[-N_select:]
p0 = unique_chain[top_indices]
