...
class JaxDistanceDensityTable:
    ...
    def source_pdf(self, ds_pc: float) -> jnp.ndarray:
        val = jnp.interp(ds_pc, self.distance_pc, self.source_density, left=0.0, right=0.0)
        return jnp.where(self.source_norm > 0.0, val / self.source_norm, 0.0)

    def lens_pdf_given_source(self, dl_pc: float, ds_pc: float) -> jnp.ndarray:
        norm = self._lens_integral_until(ds_pc)
        val = jnp.interp(dl_pc, self.distance_pc, self.lens_density_total, left=0.0, right=0.0)
        return jnp.where((ds_pc > dl_pc) & (norm > 0.0), val / norm, 0.0)

    def _lens_integral_until(self, ds_pc: float) -> jnp.ndarray:
    ...
    def component_fractions(self, dl_pc: float) -> jnp.ndarray:
        vals = jnp.array(
            [
                jnp.interp(
                    dl_pc,
...
class JaxMurelHistogram:
    ...
    def densities(self, dl_pc: float, ds_pc: float, mu: float, phi: float) -> tuple[jnp.ndarray, jnp.ndarray]:
        idx = self._nearest_pair_index(dl_pc, ds_pc)
    ...
    def _nearest_pair_index(self, dl_pc: float, ds_pc: float) -> jnp.ndarray:
        target = jnp.array([ds_pc, dl_pc])
...
class JaxHistogramDensity:
    ...
    def density_mu_phi(self, mass: float, dl_pc: float, ds_pc: float, mu: float, phi: float) -> jnp.ndarray:
        p_mass = self.mass_density_given_dl(mass, dl_pc)
        p_dl = self.distance.lens_pdf_given_source(dl_pc, ds_pc)
        p_ds = self.distance.source_pdf(ds_pc)
        p_mu, p_phi = self.murel.densities(dl_pc, ds_pc, mu, phi)
        return p_mass * p_dl * p_ds * p_mu * p_phi

    def log_density_mu_phi(self, mass: float, dl_pc: float, ds_pc: float, mu: float, phi: float) -> jnp.ndarray:
        density = self.density_mu_phi(mass, dl_pc, ds_pc, mu, phi)
        return jnp.where(density > 0.0, jnp.log(density), -jnp.inf)

    def mass_density_given_dl(self, mass: float, dl_pc: float) -> jnp.ndarray:
    ...
    def component_fractions(self, dl_pc: float) -> dict[str, jnp.ndarray]:
