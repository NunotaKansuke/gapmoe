...
double MU_N_MIN = MU_MIN;
double MU_N_MAX = MU_MAX;
double MU_E_MIN = MU_MIN;
double MU_E_MAX = MU_MAX;

int MU_BIN_NUM = (int)((MU_MAX - MU_MIN) / MU_BIN_WIDTH);
int MU_N_BIN_NUM = (int)((MU_N_MAX - MU_N_MIN) / MU_BIN_WIDTH);
int MU_E_BIN_NUM = (int)((MU_E_MAX - MU_E_MIN) / MU_BIN_WIDTH);
...
printf("# BIN_INFO: MU_N_MIN= %.2f MU_N_MAX= %.2f MU_E_MIN= %.2f MU_E_MAX= %.2f MU_BIN_WIDTH= %.2f ", 
       MU_N_MIN, MU_N_MAX, MU_E_MIN, MU_E_MAX, MU_BIN_WIDTH);
printf("MU_N_BIN_NUM= %d MU_E_BIN_NUM= %d ", MU_N_BIN_NUM, MU_E_BIN_NUM);
...
//-------------------------------------------------//
  ...
  double **mu_hist = (double **)calloc(MU_BIN_NUM, sizeof(double *));
  for (int i = 0; i < MU_BIN_NUM; ++i) {
    mu_hist[i] = (double *)calloc(MU_BIN_NUM, sizeof(double));
  }
  int total_count = 0;
...
//nunota count------------
     double muN =  murelb * cosPA + murell * sinPA;
     double muE = -murelb * sinPA + murell * cosPA;
     
     if (muN >= MU_MIN && muN < MU_MAX && muE >= MU_MIN && muE < MU_MAX) {
         int i_muN = (int)((muN - MU_MIN) / MU_BIN_WIDTH);
         int i_muE = (int)((muE - MU_MIN) / MU_BIN_WIDTH);
     
         if (i_muN >= 0 && i_muN < MU_BIN_NUM && i_muE >= 0 && i_muE < MU_BIN_NUM) {
             mu_hist[i_muN][i_muE] += 1.0;
             total_count++;
         }
     }
     continue;     
...
//------nunota output
  for (int i = 0; i < MU_BIN_NUM; ++i) {
      double muN_center = MU_MIN + (i + 0.5) * MU_BIN_WIDTH;
      for (int j = 0; j < MU_BIN_NUM; ++j) {
          double muE_center = MU_MIN + (j + 0.5) * MU_BIN_WIDTH;
          double norm_val = (total_count > 0) ? mu_hist[i][j] / total_count : 0.0;
  
          printf("%.3f %.2f %.3f %.3f %.6f\n", DS_bin_center, DL_bin_center, muN_center, muE_center, norm_val);
      }
  }
  for (int i = 0; i < MU_BIN_NUM; ++i) {
    free(mu_hist[i]);
