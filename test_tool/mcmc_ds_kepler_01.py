...
init_params = np.array([t0,tE,u0,rho,q,s,alpha,piEN,piEE,gamma1,gamma2,gamma3,sz,Ds])


# In[5]:
def calc_as(theta,thS):
    t0 = theta[0]
    tE     = theta[1]
    u0 = theta[2]
    rho    = theta[3]
    ...
    a = -mu/(2*eps)
    a_s = a/jnp.sqrt(s**2 + sz**2)
    return a_s


def calc_ln_like(theta,thS,source_flux=False):
    _t0 = theta[0]
    _tE     = theta[1]
    _u0 = theta[2]
    _rho    = theta[3]
