...
@pytest.fixture
def fake_genulens_root(tmp_path):
    root = tmp_path / "genulens"
    (root / "pre_gapmoe").mkdir(parents=True)
    return root
...
def test_resume_reopens_an_existing_directory_from_metadata(tmp_path, fake_genulens_root):
    directory = tmp_path / "event-001"
    directory.mkdir()
    paths = {name: directory / f"{name}.dat" for name in ("mass", "rho", "murel")}
    for path in paths.values():
