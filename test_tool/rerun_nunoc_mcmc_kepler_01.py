...
def calc_ln_prior(theta, thS, vEarth):
    t0, u0, q, ML, DL, DS, murel_N_hel, murel_E_hel,orbital_radi, e, cos_i, Om_NE, om, nu = parametrics.physical_to_lightcurve_kepler(theta, thS, vEarth)
...
pre_path = f"/moao38_7/nunota/gapmoe/test_result/copy/simu_01_kepler_chain_01.h5"
pre_sampler = emcee.backends.HDFBackend(pre_path)
pre_chain = pre_sampler.get_chain(flat=False)
pre_prob = pre_sampler.get_log_prob(flat=False)

p0 = pre_chain[-1,np.where(pre_prob[-1,:]>-8775),0][0]
